...
public class ReliableDataOutputChannel2Tests : IDisposable
{
    ...
    private readonly ReliableDataOutputChannel2 _channel;

    public ReliableDataOutputChannel2Tests()
    {
        ...
        _channel = new ReliableDataOutputChannel2(handler, SpanPool, MAX_DATA_LENGTH + sizeof(ushort));
    }
    ...
    public async Task TestRepeatsDataOnAckFailure()
    {
        ...
        // Don't acknowledge
        await Task.Delay(ReliableDataOutputChannel2.ACK_WAIT_MILLISECONDS + 100);
        _channel.RunTick(CancellationToken.None);
        AssertReceivedPacketsEqualBuffer(_netInterface, packet, true);
    }
