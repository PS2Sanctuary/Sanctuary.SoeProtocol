...
pub fn hash(mut data: &[u8], seed: u32) -> u32 {
    const UNROLL: usize = 4;
    const BYTES_AT_ONCE: usize = 16 * UNROLL;

    let mut crc: u32 = CRC32_LOOKUP[0][(!seed & 0xFF) as usize];
    crc ^= 0x00FFFFFF;
    let mut index: u32 = (seed >> 8) ^ crc;
    crc = (crc >> 8) & 0x00FFFFFF;
    crc ^= CRC32_LOOKUP[0][(index & 0xFF) as usize];
    index = (seed >> 16) ^ crc;
    crc = (crc >> 8) & 0x00FFFFFF;
    crc ^= CRC32_LOOKUP[0][(index & 0xFF) as usize];
    index = (seed >> 24) ^ crc;
    ...
    while data.len() >= BYTES_AT_ONCE {
        for _ in 0..UNROLL {
            crc = CRC32_LOOKUP[0x0][data[0xf] as usize]
                ^ CRC32_LOOKUP[0x1][data[0xe] as usize]
                ^ CRC32_LOOKUP[0x2][data[0xd] as usize]
                ^ CRC32_LOOKUP[0x3][data[0xc] as usize]
                ^ CRC32_LOOKUP[0x4][data[0xb] as usize]
                ^ CRC32_LOOKUP[0x5][data[0xa] as usize]
                ^ CRC32_LOOKUP[0x6][data[0x9] as usize]
                ^ CRC32_LOOKUP[0x7][data[0x8] as usize]
                ^ CRC32_LOOKUP[0x8][data[0x7] as usize]
                ^ CRC32_LOOKUP[0x9][data[0x6] as usize]
                ^ CRC32_LOOKUP[0xa][data[0x5] as usize]
                ^ CRC32_LOOKUP[0xb][data[0x4] as usize]
                ^ CRC32_LOOKUP[0xc][data[0x3] as usize ^ ((crc >> 0x18) & 0xFF) as usize]
                ^ CRC32_LOOKUP[0xd][data[0x2] as usize ^ ((crc >> 0x10) & 0xFF) as usize]
                ^ CRC32_LOOKUP[0xe][data[0x1] as usize ^ ((crc >> 0x08) & 0xFF) as usize]
                ^ CRC32_LOOKUP[0xf][data[0x0] as usize ^ ((crc >> 0x00) & 0xFF) as usize];
            data = &data[16..];
        }
    }

    for &element in data.iter() {
        let lookup_index = ((crc as u8) ^ element) as usize;
        let lookup_element = CRC32_LOOKUP[0][lookup_index];
        crc = lookup_element ^ (crc >> 8);
    }
...
mod tests {
    ...
    #[test]
    fn test_vector_1() {
        const HASH: u16 = 52395;
        let result = hash(&DATA, SEED) as u16;
        assert_eq!(HASH, result)
    }
}

static CRC32_LOOKUP: [[u32; 256]; 16] = [
    [
