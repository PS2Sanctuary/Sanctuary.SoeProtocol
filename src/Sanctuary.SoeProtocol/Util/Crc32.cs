...
public static class Crc32
{
    ...
    private static unsafe uint Crc32BigEndian(byte* dataPtr, int count, uint seed)
    {
        const int unroll = 4;
        const int bytesAtOnce = 16 * unroll;
    ...
    /// <summary>
    /// Swaps the endianness of a uint value.
    /// </summary>
    /// <param name="x">The value.</param>
    /// <returns>The value with its endianness reversed.</returns>
    private static uint ReverseEndianness(uint x)
        => (x >> 24) |
           ((x >> 8) & 0x0000FF00) |
           ((x << 8) & 0x00FF0000) |
           (x << 24);
