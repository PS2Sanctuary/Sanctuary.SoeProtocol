...
/// The pre-scheduled seed used by the `hash` function.
_scheduled_seed: u32 = 0,

pub fn init(seed: u32) Crc32 {
    return Crc32{ ._scheduled_seed = scheduleSeed(seed) };
}

/// Hashes data using a custom variant of the CRC32 algorithm.
pub fn hash(self: Crc32, data: []const u8) u32 {
    const unroll = 4;
    const bytes_at_once = 16 * unroll;

    var crc: u32 = self._scheduled_seed;

    const lookup_0: [*]const u32 = &LOOKUP;
    // Forcing the alignment of 1 to be OK here isn't ideal. We should benchmark whether processing one byte
    // at a time is more efficient than processing four bytes with 1-byte alignment
    var uint_ptr: [*]align(1) const u32 = @ptrCast(data.ptr);
    const uint_end_ptr: [*]align(1) const u32 = uint_ptr + (data.len / bytes_at_once) * (bytes_at_once / 4);

    const lookup_1: [*]const u32 = lookup_0 + 0x100;
    const lookup_2: [*]const u32 = lookup_0 + 0x200;
    const lookup_3: [*]const u32 = lookup_0 + 0x300;
    const lookup_4: [*]const u32 = lookup_0 + 0x400;
    const lookup_5: [*]const u32 = lookup_0 + 0x500;
    const lookup_6: [*]const u32 = lookup_0 + 0x600;
    const lookup_7: [*]const u32 = lookup_0 + 0x700;
    const lookup_8: [*]const u32 = lookup_0 + 0x800;
    const lookup_9: [*]const u32 = lookup_0 + 0x900;
    const lookup_10: [*]const u32 = lookup_0 + 0xA00;
    const lookup_11: [*]const u32 = lookup_0 + 0xB00;
    const lookup_12: [*]const u32 = lookup_0 + 0xC00;
    const lookup_13: [*]const u32 = lookup_0 + 0xD00;
    const lookup_14: [*]const u32 = lookup_0 + 0xE00;
    const lookup_15: [*]const u32 = lookup_0 + 0xF00;

    while (@intFromPtr(uint_ptr) < @intFromPtr(uint_end_ptr)) {
        for (0..unroll) |_| {
            // Little endian
            const one: u32 = (uint_ptr[0]) ^ crc;
            uint_ptr += 1;
            const two: u32 = uint_ptr[0];
            uint_ptr += 1;
            const three: u32 = uint_ptr[0];
            uint_ptr += 1;
            const four: u32 = uint_ptr[0];
            uint_ptr += 1;
    ...
    for (((data.len / bytes_at_once) * bytes_at_once)..data.len) |i| {
...
fn scheduleSeed(seed: u32) u32 {
    var state: u32 = LOOKUP[~seed & 0xff];
    state ^= 0x00FFFFFF;
    var index: u32 = (seed >> 8) ^ state;
    state = (state >> 8) & 0x00FFFFFF;
    state ^= LOOKUP[index & 0xFF];
    index = (seed >> 16) ^ state;
    state = (state >> 8) & 0x00FFFFFF;
    state ^= LOOKUP[index & 0xFF];
    index = (seed >> 24) ^ state;
...
test "Test CRC32 Hashing" {
    const crc = Crc32.init(1858192374);
    const data = [_]u8{  };

    try std.testing.expectEqual(3893349547, crc.hash(&data));
}
